...
bool StaticData::booleanFromKey(const string& key)        //根据键值得到bool类型数据
{
	return _dictionary->valueForKey(key.c_str())->boolValue();
}
CCPoint StaticData::pointFromKey(const string& key)       //根据键值得到point类型数据 
{
	return CCPointFromString(_dictionary->valueForKey(key.c_str())->getCString());
}
CCRect StaticData::rectFromKey(const string& key)          //根据键值得到rect类型数据
{
