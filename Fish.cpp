...
Fish* Fish::create(FishType type/* = k_Fish_Type_SmallFish */)//创建鱼的类型
{
	Fish* fish = new Fish;
...
bool Fish::init(FishType type /* = k_Fish_Type_SmallFish */)//鱼的类型
{
	do 
	{
		if (!CCNode::init())
...
int Fish::getScore(void)//获得鱼的得分
{
	return 0;
}
int Fish::getSpeed(void)//获取鱼的游速
{
	return 200;
}
